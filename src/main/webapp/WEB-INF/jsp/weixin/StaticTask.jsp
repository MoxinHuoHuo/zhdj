...
$("#uon").on('click',function(){
    location.href="${pageContext.request.contextPath}/base/goURL/weixin/SelectUonTask"
});

function loadOneColumn() {
var pie1 = echarts.init(document.getElementById("pie1"));
pie1.setOption({
  color: ['#F70044', '#F6D600', '#11CD86'],
  title : {
    text: "组织任务统计图",
    x: 'center'
  },
...
var names = [];    //类别数组（用于存放饼图的类别）已完成，未完成，超时完成
var brower = [];
$.ajax({
  type:'post',
  url:'${pageContext.request.contextPath}/Count',
  data:{startTime:$('#hid1').val(),lastTime:$('#hid2').val(),receiveUnit:$('#hid').val(),sentId:RelUserLoginId},
  dataType:'json',
      ...
      if(index==0){
        ...
        success:function(data){
          $.each(data.Arrays,function(index,item){
            if(index==0){
              var numberArray=item.numberArray;
              location.href="${pageContext.request.contextPath}/base/goURL/weixin/StaticUonfsh?numberArray="+numberArray
            }
          })
        }
        })
